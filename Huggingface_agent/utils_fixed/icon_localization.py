...
def remove_boxes(boxes_filt, confidence, size, iou_threshold=0.5):
    boxes_to_remove = set()
    ...
    confidence = [box for idx, box in enumerate(confidence) if idx not in boxes_to_remove]
    
    return boxes_filt, confidence
...
def det(input_image_path, caption, groundingdino_model, processor, device, box_threshold=0.05, text_threshold=0.5):
    ...
    confidence = result['scores'].detach().cpu().tolist()
    boxes_filt = sorted(boxes_filt, key = lambda x: confidence[boxes_filt.index(x)], reverse = True)
    ...
    filtered_boxes, filtered_confidence = remove_boxes(boxes_filt, confidence, size)  # [:9]
