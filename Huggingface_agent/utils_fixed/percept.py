...
@deprecated("has changed into another version")
def get_perception_infos_origin(
...
def get_perception_infos(
    ...
    img_list = [temp_file_prefix+f'/{fileName}_part_1.png', temp_file_prefix+f'/{fileName}_part_2.png',
                temp_file_prefix+f'/{fileName}_part_3.png', temp_file_prefix+f'/{fileName}_part_4.png', 
                file]
    img_x_list = [0, total_width/2, 0, total_width/2, 0]
    img_y_list = [0, 0, total_height/2, total_height/2, 0]
    coordinates = []
    padding = total_height * 0.0025  # 10
    ...
    confidences = []
    for i, img in enumerate(img_list):
        sub_coordinates, sub_confidence = det(img, "icon", groundingdino_model,preprocessor, device)
        for coordinate in sub_coordinates:
            coordinate[0] = int(max(0, img_x_list[i] + coordinate[0] - padding))
            coordinate[2] = int(min(total_width, img_x_list[i] + coordinate[2] + padding))
            coordinate[1] = int(max(0, img_y_list[i] + coordinate[1] - padding))
        ...
        #sub_coordinates = merge_all_icon_boxes(sub_coordinates)
        coordinates.extend(sub_coordinates)
        confidences.extend(sub_confidence)
        
    #merged_icon_coordinates = coordinates
    merged_icon_coordinates, merged_confidences = merge_all_icon_boxes(coordinates, confidences)
    origin_file_path = Path(file)
    output_file_path = Path(output_file_prefix) / origin_file_path.parent.name / ("dino_" + origin_file_path.name)
    output_file_path.parent.mkdir(parents=True, exist_ok=True)
    draw_coordinates_boxes_on_image(file, copy.deepcopy(merged_icon_coordinates), output_file_path, font_path)
    
    return merged_icon_coordinates, merged_confidences
    ...
    # return perception_infos, image_box, 0